  ...
  return (
    <section className="grid gap-4">
      ...
      <PreferenceSection title="Preferences">
        ...
        <PreferenceField label="Display mode">
          <select
            className="rounded-2xl border border-slate-200 bg-white px-3 py-2"
            ...
            onChange={(event) =>
              setDraft((current) => ({
                ...current,
                displayMode: event.target.value as UserPreferences["displayMode"]
              }))
            }
          >
            <option value="icon-only">Icon only</option>
            <option value="icon-plus-percent">Icon + percent</option>
            <option value="multi-dimension">Multiple dimensions</option>
          </select>
        </PreferenceField>
